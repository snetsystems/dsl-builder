...
import {SavedObjectsClientCommon, UI_SETTINGS} from '../..'
...
import { DataSourceAttributes, SavedObject } from 'src/type'
...
export class IndexPatternsService {
  ...
  findDataSourceByTitle = async (title: string, size: number = 10) => {
    const savedObjectsResponse = await this.savedObjectsClient.find<any>({
      type: 'data-source',
      fields: ['title'],
      search: title,
      searchFields: ['title'],
  ...
  savedObjectToSpec = (savedObject: any): IndexPatternSpec => {
    const {
      id,
      version,
      attributes: {
  ...
  get = async (
    ...
    try {
      ...
      if (err instanceof IndexPatternMissingIndices) {
        ...
        this.onError(err as Error, {
          title: 'data.indexPatterns.fetchFieldErrorTitle',
        })
      }
    }
    ...
    if (isSaveRequired) {
      try {
        ...
        this.onError(err as Error, {
          title: 'data.indexPatterns.fetchFieldSaveErrorTitle',
        })
      }
    }
...
export type IndexPatternsContract = any
