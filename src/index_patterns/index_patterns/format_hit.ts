...
export function formatHitProvider(
  ...
  function formatHit(hit: Record<string, any>, type: any = 'html') {
    ...
    _.forOwn(
      indexPattern.flattenHit(hit),
      function (val: any, fieldName?: string) {
        // sync the formatted and partial cache
        if (!fieldName) {
          return
        }
        const formatted =
          partials[fieldName] == null
            ? convert(hit, val, fieldName, type)
            : partials[fieldName]
