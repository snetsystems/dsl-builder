...
const dateComparators = {
  gt:
    'boolean gt(Supplier s, def v) {return s.get().toInstant().isAfter(Instant.parse(v))}',
  gte:
  ...
  lt:
    'boolean lt(Supplier s, def v) {return s.get().toInstant().isBefore(Instant.parse(v))}',
}
...
const hasRangeKeys = (params: RangeFilterParams) =>
  Boolean(
    keys(params).find((key: string) =>
      ['gte', 'gt', 'lte', 'lt', 'from', 'to'].includes(key)
    )
  )
...
const formatValue = (field: IFieldType, params: any[]) =>
  map(
    params,
    (val: any, key: string) => get(operators, key) + format(field, val)
  ).join(' ')

const format = (field: IFieldType, value: any) =>
  field && field.format && field.format.convert
...
export const buildRangeFilter = (
  ...
  const totalInfinite = ['gt', 'lt'].reduce((acc: number, op: any) => {
    ...
    const isInfinite =
      typeof params[key] !== 'bigint' &&
      Math.abs(get(params, key) as number) === Infinity
  ...
  if (totalInfinite === OPERANDS_IN_RANGE) {
    ...
    filter.script.script.params.value = formatValue(
      field,
      filter.script.script.params
    )
...
export const getRangeScript = (
  ...
  let script = map(
    knownParams,
    (val: any, key: string) =>
      '(' + field.script + ')' + get(operators, key) + key
  ).join(' && ')
