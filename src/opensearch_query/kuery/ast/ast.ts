...
// @ts-ignore
const parseKuery: Function = require('./_generated_/kuery.js').parse;
...
export const fromKueryExpression = (
  ...
  try {
    ...
    if (error instanceof Error && error.name === 'SyntaxError') {
      throw new DQLSyntaxError(error as any, expression)
    } else {
      throw error
    }
  }
...
export const doesKueryExpressionHaveLuceneSyntaxError = (
  ...
  try {
    ...
    return e instanceof Error && e.message.startsWith('Lucene')
  }
}
