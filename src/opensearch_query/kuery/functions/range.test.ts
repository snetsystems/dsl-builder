  ...
  describe('range', () => {
    ...
    describe('buildNodeParams', () => {
      ...
      test('arguments should contain the provided params as named arguments', () => {
        const givenParams: RangeFilterParams = {
          gt: 1000,
          lt: 8000,
          format: 'epoch_millis',
        }
    ...
    describe('toOpenSearchQuery', () => {
      test("should return an OpenSearch range query for the node's field and params", () => {
        ...
        const node = nodeTypes.function.buildNode('range', 'bytes', {
          gt: 1000,
          lt: 8000,
        })
      ...
      test('should support wildcard field names', () => {
        ...
        const node = nodeTypes.function.buildNode('range', 'byt*', {
          gt: 1000,
          lt: 8000,
        })
        const result = range.toOpenSearchQuery(node, indexPattern)
      ...
      test('should support scripted fields', () => {
        const node = nodeTypes.function.buildNode('range', 'script number', {
          gt: 1000,
          lt: 8000,
        })
        const result = range.toOpenSearchQuery(node, indexPattern)
