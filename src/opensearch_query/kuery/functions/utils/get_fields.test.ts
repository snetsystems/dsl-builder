...
describe('getFields', () => {
  ...
  describe('field names without a wildcard', () => {
    ...
    test('should not match a wildcard in a literal node', () => {
      ...
      const actual = getFields(
        nodeTypes.literal.buildNode('fo*'),
        indexPatternWithWildField
      )
      expect(actual).toEqual([])
    })
  })
  ...
  describe('field name patterns with a wildcard', () => {
    ...
    test('should return all fields that match the pattern in an array', () => {
      ...
      expect(
        results!.find((field: IFieldType) => field.name === 'machine.os')
      ).toBeDefined()
      expect(
        results!.find((field: IFieldType) => field.name === 'machine.os.raw')
      ).toBeDefined()
    })
  })
})
