  ...
  describe('geoPolygon', () => {
    ...
    describe('toOpenSearchQuery', () => {
      test('should return an OpenSearch geo_polygon query representing the given node', () => {
        ...
        ;(result.geo_polygon.geo as any).points.forEach(
      ...
      test('should throw an error for scripted fields', () => {
        ...
        expect(() =>
          geoPolygon.toOpenSearchQuery(node, indexPattern)
        ).toThrowError(/Geo polygon query does not support scripted fields/)
      })
