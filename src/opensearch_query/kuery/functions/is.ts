...
import {IFieldType} from 'src/index_patterns/fields/types'
...
export function toOpenSearchQuery(
  ...
  const value = !isUndefined(valueArg)
  ...
  const isAllFieldsQuery =
    (fullFieldNameArg.type === 'wildcard' &&
      ((fieldName as unknown) as string) === '*') ||
  ...
  const queries = fields!.reduce((accumulator: any, field: IFieldType) => {
    ...
    */
    ...
    } else if (field.esTypes?.length === 1 && field.esTypes.includes('keyword')) {
      return [
        ...accumulator,
        wrapWithNestedQuery({
          term: {
