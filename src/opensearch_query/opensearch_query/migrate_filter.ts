...
function isDeprecatedMatchPhraseFilter(
  filter: any
): filter is DeprecatedMatchPhraseFilter {
  const fieldName =
    filter.query && filter.query.match && Object.keys(filter.query.match)[0]

  return Boolean(
    fieldName && get(filter, ['query', 'match', fieldName, 'type']) === 'phrase'
  )
}
...
export function migrateFilter(filter: Filter, indexPattern?: IIndexPattern) {
  if (isDeprecatedMatchPhraseFilter(filter)) {
    ...
    const params: Record<string, any> = get(filter, [
      'query',
      'match',
      fieldName,
    ])
